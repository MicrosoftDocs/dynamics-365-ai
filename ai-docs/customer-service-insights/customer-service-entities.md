...
OwningBusinessUnit | Lookup | A unique identifier for the business unit that owns the case. AI for Customer Service Insights uses this attribute to look up the business unit names from the [BusinessUnit](#BKMK-businessunit-entity) entity and generate the Business Unit filter values on each dashboard.
