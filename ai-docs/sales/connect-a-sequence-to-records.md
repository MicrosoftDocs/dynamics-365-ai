...
4. Select the records to which you want to connect the sequence. In this example, we select **Nancy Anderson**<!--note from editor: It looks like this name was supplied with sample data, but it's an actual Microsoft employee name. Might be worth pointing this out to someone (not sure who!)?--> and **Maria Campbell**.   
    > [!div class="mx-imgBorder"]
...
5. Select **+ New step**.   
6. In the **Search connectors and actions** box, select **Common Data Services**, and then search for and add the action **Perform an unbound action**.<!--note from editor: Edit okay? It doesn't look like you actually enter **Common Data Servcies** here, rather you select its icon.-->     
    > [!div class="mx-imgBorder"]
...
7.	In the **Perform an unbound action** step, select the **Action Name** as **msdyn_ConnectSequence**.     
    ...
    Enter the following additional information:<!--note from editor: Edit okay? "As required" implies that it won't always be required. If that's actually what you mean, this should be something like "...as required for your scenario."-->     
...
9. (Optional) Select **Test** to test your flow.     
    Ensure that all the configured steps are working as required. The test feature runs and validates each step in the flow, and highlights any step that contains an error.<!--note from editor: Edit okay? "Highlights" needs a direct object.--> You must resolve the error to proceed.    
